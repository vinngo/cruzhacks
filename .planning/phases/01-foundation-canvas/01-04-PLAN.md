...
<task type="auto">
  ...
  <action>
    ...
    **PIVOT NOTE:** During execution, react-resizable-panels was replaced with simple Tailwind flexbox layout (w-1/5 and w-4/5) due to persistent rendering issues. Panels are now fixed width (non-resizable), which still achieves the core requirement of 20/80 split and clear visual hierarchy. Welcome hint text was also removed due to tldraw text shape API incompatibility.
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    3. **Workspace Layout:**
       - Verify two-panel layout displays
       - Verify left panel (20% width) shows "Problem" heading and placeholder
       - Verify right panel (80% width) shows tldraw canvas
       - Verify panels fill full viewport height
       - Verify clean separation between panels (no resize handle - simplified to fixed layout)

    4. **Canvas Functionality:**
       - Verify canvas displays (empty white canvas)
       ...
       - Note: Welcome hint text was removed due to tldraw API incompatibility
    ...
    5. **Persistence:**
       ...
       - Refresh page
       - Verify canvas is empty (blank white canvas)
...
**Workspace:**
- [ ] Two-panel layout renders (flexbox, not resizable)
- [ ] Problem panel (20% width) on left
- [ ] Canvas (80% width) on right
- [ ] Panels fill full viewport height
- [ ] Visual hierarchy clear (canvas dominant)

**Canvas:**
- [ ] tldraw renders with proper styling
- [ ] Canvas starts empty (no welcome hint - removed due to API issue)
