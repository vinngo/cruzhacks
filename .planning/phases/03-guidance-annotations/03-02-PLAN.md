 ...
 * Simplified: Uses viewport bounds, not shape overlap detection (MVP approach)
 */
...
- useMemo recalculates positions when annotations or editor change
- Positions calculated sequentially to avoid overlaps between multiple annotations
- Simplified approach: viewport bounds only (not shape overlap detection for MVP)
...
4. Fallback stacking if all corners occupied
...
- [ ] Positioning algorithm uses viewport bounds
