...
must_haves:
  truths:
    ...
    - "Annotation overlay cards display proposed annotation details"
  artifacts:
    ...
    - path: "components/TldrawEditor.tsx"
      provides: "Editor instance accessible via ref for annotation positioning"
...
<objective>
Implement collapsible chat sidebar, AI structured annotation proposals, and expose editor instance for positioning.

Purpose: Enable users to maximize canvas space by collapsing chat, and set up AI system to propose canvas annotations (questions and hints) in structured format for rendering. Establishes foundation for Phase 3's core value: AI guides via interactive canvas annotations. Expose editor instance for smart annotation positioning in later plans.

Output: Collapsible chat sidebar with smooth transitions, AI generating structured annotation proposals (type, text, position), type definitions for annotation system, TldrawEditor ref exposing getEditor() method.
</objective>
...
<task type="auto">
  <name>Implement AI annotation proposal system and expose editor instance</name>
  <files>lib/types/annotations.ts, app/api/chat/route.ts, lib/ai/prompt.ts, components/TldrawEditor.tsx</files>
  <action>
...
**Update components/TldrawEditor.tsx:**
Expose editor instance via ref for annotation positioning:
...
- Tool calls accessible via message.parts array with part.type === 'tool-call'
- Client will extract tool calls and render as annotation overlays (next plan)
- Zod schema validates AI's output structure
- getEditor() enables annotation positioning algorithm to access tldraw Editor instance
  </action>
  <verify>
    ...
    editorRef.current.getEditor() returns Editor instance
    getEditor() returns null when editor not mounted
  </verify>
  <done>AI generates structured annotation proposals via tool calling, types defined, system prompt updated with annotation guidance, editor instance accessible via ref</done>
</task>
...
**Editor access:**
1. TldrawEditorRef exports getEditor() method
2. getEditor() returns Editor instance when mounted
3. getEditor() returns null when not mounted
...
- [ ] TldrawEditorRef exposes getEditor() method
- [ ] getEditor() returns Editor instance for positioning calculations
