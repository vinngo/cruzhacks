...
must_haves:
  ...
  key_links:
    - from: "components/workspace/ChatPanel.tsx"
      to: "/api/chat"
      via: "useChat hook from ai/react"
      pattern: "useChat.*ai/react"
...
- Use useChat hook from 'ai/react' for built-in streaming support
- Import: `import { useChat } from 'ai/react';`
- Hook provides: messages, input, handleInputChange, handleSubmit, isLoading
- This is REQUIRED (not optional) because Task 2 uses toDataStreamResponse() which returns the data stream format that useChat() expects
- Manual fetch with ReadableStream will NOT work with the API's response format
...
- Pass problem data in body field of useChat configuration
...
Export default ChatPanel component.
  ...
  <done>ChatPanel.tsx exists with message display, input using useChat hook, and problem context integration</done>
</task>
